...
class Decoder(nn.Module):
    ...
    def __init__(self, T, decoder_num_hidden, encoder_num_hidden):
        ...
        self.fc_final = nn.Linear(decoder_num_hidden + encoder_num_hidden, 24)
...
class DA_RNN(nn.Module):
    ...
    def __init__(self, X, y, T,
        ...
        # Training set
        self.train_timesteps =int((self.X.shape[0]*0.7//24)*24)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            while (idx < self.train_timesteps):
                ...
                indices1 = indices+self.T
                y_gt = np.zeros((len(indices1), self.T))
                for i in range(len(indices1)):
                    y_gt[i, :] = self.y[indices1[i]:indices1[i] + self.T]
    ...
    def test(self, on_train=False):
        ...
        while i < len(y_pred):
            ...
            # print("赋值的范围为:{}:{}".format(i,i+self.batch_size))
            i += self.batch_size
